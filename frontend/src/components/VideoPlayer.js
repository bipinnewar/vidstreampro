...
const VideoPlayer = () => {
  ...
  const submitComment = async (e) => {
    ...
    try {
      const res = await apiClient.post(`/api/videos/${id}/comments`, { text: commentText });
      setComments([res.data, ...comments]);
      setCommentText('');
    } catch (err) {
  ...
  const submitRating = async () => {
    try {
      const res = await apiClient.post(`/api/videos/${id}/rate`, { score: rating });
      setVideo({ ...video, ratingAvg: res.data.ratingAvg, ratingCount: res.data.ratingCount });
      toast.success('Thanks for rating!');
    } catch (err) {
      toast.error(err.response?.data?.error || 'Failed to rate video');
    }
  };
    ...
    <Container className="mt-3">
      ...
      <Row>
        <Col md={8}>
          ...
          <p>
            <strong>Rating:</strong> {video.ratingAvg?.toFixed(2) || 'N/A'} ({video.ratingCount || 0} ratings)
          </p>
